...
public class aoc1 {
    public static void main(String[] args){
        ...
        if(args[0].equals("2")){
            int top = 0;
            int bottom = vals.length - 1;
            int sum = vals[top] + vals[bottom];
            while(sum != 2020){
                if(sum < 2020)
                    top++;
                else
                    bottom--;
                sum = vals[top] + vals[bottom];
            }
            System.out.println(vals[top] * vals[bottom]);
        }
        if(args[0].equals("3")){
            //probs a dynamic programming solution to this but no thanks
            for(int a = 0; a < vals.length; a++){
                ...
                while(sum !=2020){
                    ...
                    sum = vals[a] + vals[b] + vals[c];
                }
                System.out.println(vals[a] * vals[b] * vals[c]);
                return;
            }
        }
    }
